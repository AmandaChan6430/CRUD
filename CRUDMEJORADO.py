...
def siguiente():
    ...
    tk.Label(ventana5,
             text=" Si esta seguro de que los datos ingresados sean correctos presione finalizar, de lo contrario presione regresar  ",
             bg="beige",
    ...
    tk.Label(ventana5, text="     ", bg="paleturquoise",
             fg="black", width="100", height="1", font=("Bahnschrift", 8)).pack()
    tk.Button(ventana5, text=" Finalizar ", bg="beige", command=lambda: [ventana5.destroy, ventanajaja.destroy]).pack(
        padx=5, pady=5)
...
def Inicio_Sesion():
    def valid():
        ...
        if name3 != name4:
            ...
            tK = tk.Label(ventana13,
                          text=""" Inicio de sesion exitoso, presione "continuar" para modificar su equipo """,
                          bg="lightcyan",
...
def ventana_final():
    ...
    Button(ventana, bg="lightcyan", text="Agregar Jugador", command=agregar_alumno).grid(row=0, column=2, padx=5,
                                                                                         pady=5)
    Button(ventana, bg="lightcyan", text="Actualizar Jugador", command=actualizar_alumno).grid(row=1, column=2, padx=5,
                                                                                               pady=5)
    Button(ventana, bg="lightcyan", text="Eliminar Jugador", command=eliminar_alumno).grid(row=2, column=2, padx=5,
                                                                                           pady=5)
